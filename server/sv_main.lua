...
ESX.RegisterServerCallback('av_vipmenu:getBoutiqueId', function(src, cb, playerId)
    cb(Config.getPlayerBoutiqueId(playerId))
end)
...
RegisterNetEvent('av_vipmenu:changeWeaponTint')
AddEventHandler('av_vipmenu:changeWeaponTint', function (weapon, tint)
...
ESX.RegisterCommand({'addvip', 'vipadd'}, 'admin', function(xPlayer, args, showError)
    if args["idboutique"] ~= nil then
        ...
        xPlayer.showNotification(string.format(Config.Strings.YOU_GAVE_VIP,args["idboutique"]))
    end
...
ESX.RegisterCommand({'removevip', 'vipremove'}, 'admin', function(xPlayer, args, showError)
    if args["idboutique"] ~= nil then
        ...
        xPlayer.showNotification(string.format(Config.Strings.YOU_REMOVED_VIP,args["idboutique"]))
    end
  end, false, {arguments = {{name = 'idboutique', help = "Id boutique du joueur", type = 'number'}}
})
...
CreateThread(function()
    while true do
        ...
        for _, playerId in ipairs(GetPlayers()) do
            local boutiqueId = Config.getPlayerBoutiqueId(playerId)
            if checkGroup(boutiqueId) == true then
                local xPlayer = ESX.GetPlayerFromId(playerId)
                if xPlayer ~= nil then
                    xPlayer.addAccountMoney(Config.subventionAccount, amount)
